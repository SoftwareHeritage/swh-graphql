...
from swh.model.model import OriginVisitStatus, Sha1Git
...
class LatestVisitStatusNode(BaseVisitStatusNode):
    ...
    def _get_node_data(self) -> Optional[OriginVisitStatus]:
        # self.obj.origin is the origin URL
        assert self.obj.visitId is not None
        return self.archive.get_latest_visit_status(
            ...
            require_snapshot=self.kwargs.get("requireSnapshot"),  # type: ignore
        )
...
class VisitStatusConnection(BaseConnection):
    ...
    def _get_index_cursor(self, index: int, node: BaseVisitStatusNode) -> Optional[str]:
